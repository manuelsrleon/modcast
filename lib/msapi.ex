defmodule Modcast.MSAPI do
  ...
  defp handle_game_command(%{"action" => "get_stats" }, state) do
    s = GameStateComponent.get_players()
    reply_to_game(state,"ok", s)
    {:noreply, state}
  end

  #{"action":"get_mod_info"}
  defp handle_game_command(%{"action" => "get_mod_info" }, state) do
    i = GameStateComponent.get_players()
    reply_to_game(state,"ok", i)
    {:noreply, state}
  end

  #{"action":"is_mod_available"}
  defp handle_game_command(%{"action" => "is_mod_available" }, state) do
    i = GameStateComponent.get_players()
    reply_to_game(state,"ok", i)
    {:noreply, state}
  end
  ...
  defp handle_game_command(%{"action" => "list_available_mods" }, state) do
    l= GameStateComponent.get_players()
